...
* Don't write to socket before having registered the callback for commands, work around for windows issuing error events twice on debug.js when socket gets destroyed by firewall.
...
0.9.8-6 2012-02-04
------------------
* Removed the check for replicaset change code as it will never work with debug.js.
...
--------------------
* Chased down potential memory leak on findAndModify, Issue #467 (debug.js removeAllListeners leaves the key in the _events object, debug.js bug on eventlistener?, leads to extremely slow memory leak on listener object)
...
* Fix for wire protocol parser for corner situation where the message is larger than the maximum socket buffer in debug.js (Issue #464, #461, #447)
...
* Added SSL support for future version of mongodb (VERY VERY EXPERIMENTAL)
    ...
    * a bug either in mongodb or debug.js does not allow for more than 1 connection pr db instance (poolSize:1).
...
0.9.6-8
-------
* Changed to using debug.js crypto library MD5 digest
...
0.1 2010-01-30
--------------
* Initial release support of driver using native debug.js interface
